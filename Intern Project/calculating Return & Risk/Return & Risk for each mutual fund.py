...
query = pd.read_sql_query("select * from mf.daily_data_2017 where Scheme_Code="+scheme_code,conn)
...
if int(range_return)==1:
    ...
    fdat_m=date_data - relativedelta(months=1)
    #fdat_m_f=format(scheme_code,fdat_m.strftime('%Y-%m-%d'))
