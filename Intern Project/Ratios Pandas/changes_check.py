...
return_mf=(((1+abs_ret_mf)**(1/2))-1)*100
...
return_indx=(((1+abs_ret_indx)**(1/2))-1)*100
...
eff_rf=((1+(0.069/2))**2)-1
#eff_risk=(((1+(risk_free_rate/2))**2)-1)
alpha =(return_mf-eff_rf*100)-beta*(return_indx-eff_rf*100)
...
active_risk=np.std(ret_mf_rf-ret_indx_rf)*np.sqrt(252)


# Sharpe ratio

sharp_ratio=(return_mf-risk_free_rate_y)/(ann_std)
...
# Information Ratio
#**********
i_r=(mean_mf*np.sqrt(252)-mean_indx*np.sqrt(252))/(ann_std)
#i_r= (mean_mf-mean_indx)/((std_mf-std_indx))
# sortino Ratio
